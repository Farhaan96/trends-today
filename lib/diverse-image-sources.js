...
class DiverseImageSourceManager {
  ...
  getDiverseImageUrl(filename, options = {}) {
    ...
    if (!variations || variations.length === 0) {
      console.log(`No specific variations for ${baseKey}, using intelligent fallback`);
  ...
  extractImageKey(filename) {
    ...
    const directMatch = exactMatches.find(key => cleanName.includes(key) || key.includes(cleanName));
    ...
    if (cleanName.includes('iphone-16')) {
      if (cleanName.includes('camera')) return 'iphone-16-pro-max-camera-system';
      if (cleanName.includes('intelligence')) return 'iphone-16-pro-max-apple-intelligence';
      if (cleanName.includes('a18') || cleanName.includes('bionic')) return 'iphone-16-pro-max-a18-bionic';
      if (cleanName.includes('battery')) return 'iphone-16-pro-max-battery-life';
      if (cleanName.includes('titanium') || cleanName.includes('design')) return 'iphone-16-pro-max-titanium-design';
    ...
    if (cleanName.includes('iphone-15')) {
      ...
      if (cleanName.includes('usbc') || cleanName.includes('usb-c')) return 'iphone-15-pro-max-usbc';
      if (cleanName.includes('hero')) return 'iphone-15-pro-hero';
    }
    
    if (cleanName.includes('galaxy') || cleanName.includes('samsung')) {
      if (cleanName.includes('comparison')) return 'samsung-galaxy-s24-comparison';
  ...
  getUsageStats() {
    return {
      totalVariations: Object.values(this.diverseImageDatabase).reduce((sum, arr) => sum + arr.length, 0),
