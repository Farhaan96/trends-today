...
export const demoKeywordData = {
  ...
  laptops: [
    { keyword: 'MacBook Pro 14 review', volume: 6700, kd: 71, intent: 'commercial', cpc: 3.12 },
    { keyword: 'Dell XPS 13 review', volume: 4200, kd: 68, intent: 'commercial', cpc: 2.87 },
    { keyword: 'ThinkPad X1 Carbon review', volume: 2100, kd: 59, intent: 'commercial', cpc: 2.34 },
    { keyword: 'best laptops 2025', volume: 18900, kd: 78, intent: 'commercial', cpc: 3.89 }
  ],
  headphones: [
    { keyword: 'Sony WH-1000XM5 review', volume: 5100, kd: 64, intent: 'commercial', cpc: 2.21 },
    { keyword: 'AirPods Max review', volume: 3800, kd: 69, intent: 'commercial', cpc: 2.56 },
    { keyword: 'best headphones 2025', volume: 11200, kd: 73, intent: 'commercial', cpc: 2.98 }
  ]
};
...
export const demoProductData = {
  ...
  'Samsung Galaxy S24 Ultra': {
    ...
    competitorComparisons: ['iPhone 15 Pro Max', 'Google Pixel 8 Pro', 'OnePlus 12'],
  ...
  'Google Pixel 8 Pro': {
    ...
    priceRange: '$999-$1399',
    competitorComparisons: ['iPhone 15 Pro', 'Samsung Galaxy S24', 'OnePlus 12'],
...
export const demoNewsData = [
  {
    ...
    summary: 'Apple confirms it is working on a second-generation Vision Pro with improved displays, lighter design, and significantly reduced pricing expected for Q4 2025.',
    category: 'AR/VR',
    source: 'TechCrunch',
...
export async function simulateApiDelay(minMs = 500, maxMs = 2000): Promise<void> {
  const delay = Math.random() * (maxMs - minMs) + minMs;
  await new Promise((resolve) => setTimeout(resolve, delay));
}

export function isDemoMode(): boolean {
  const hasFirecrawl = process.env.FIRECRAWL_API_KEY && !process.env.FIRECRAWL_API_KEY.includes('your-api-key');
  const hasPerplexity = process.env.PERPLEXITY_API_KEY && !process.env.PERPLEXITY_API_KEY.includes('your-api-key');
