...
class DataForSEOLite {
  async batchKeywordAnalysis(keywords: string[]) {
    console.warn('DataForSEO not configured. Returning empty keyword analysis.');
    return keywords.map((k) => ({ keyword: k, volume: 0, kd: 0, cpc: 0, competition: 'unknown', intent: 'informational', notes: 'No data' }));
  }
...
export class MCPClient {
  ...
  async getComprehensiveProductData(productName: string, category: string) {
    ...
    if (isDemoMode()) {
      ...
      const keywordData = demoKeywordData[category as keyof typeof demoKeywordData] || [];
      return { research: { ...research, productName }, keywordData: keywordData.slice(0, 5), competitorUrls: [], scrapedContent: [] };
    }
    const research = await this.perplexity.researchProduct(productName, category);
    const keywords = [`${productName} review`, `${productName} vs`, `best ${category} 2025`, `${productName} specs`, `${productName} price`];
  ...
  async generateContentIdeas(category: string, count = 10) {
    ...
    return {
      ...
      comparisons: allKeywords.filter((k: string) => k.includes('vs') || k.includes('compare')).slice(0, count),
      buyingGuides: allKeywords.filter((k: string) => k.includes('best') || k.includes('guide')).slice(0, count),
      news: allKeywords.filter((k: string) => k.includes('news') || k.includes('update')).slice(0, count),
