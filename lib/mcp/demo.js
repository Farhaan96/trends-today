...
class DemoMCPClient {
  async getComprehensiveProductData(productName, category) {
    console.log('⚠️  Running in DEMO MODE - using simulated data. Configure API keys for live data.');
    ...
    const keywordData = demoKeywordData[category] || demoKeywordData.smartphones;
    ...
    return {
      ...
      scrapedContent: [
        { success: true, data: { markdown: 'Demo competitor content about product features and performance...' } }
      ],
    };
  }
...
function isDemoMode() {
  ...
  const hasDataForSEO = process.env.DATAFORSEO_LOGIN && !process.env.DATAFORSEO_LOGIN.includes('your_login');
  
  return !hasFirecrawl || !hasPerplexity || !hasDataForSEO;
}
