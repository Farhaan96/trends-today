...
class ImagePathMapper {
  constructor(options = {}) {
    this.contentDir =
      options.contentDir || path.join(__dirname, '..', 'content');
    ...
    this.pathPatterns = {
      ...
      'product-features': {
        pattern:
          /^\/images\/products\/([^\/]+)-(camera|battery|display|design|titanium|usbc|a18|intelligence)\.jpg$/,
        directory: 'products',
        priority: 6,
      },
      // Author images
  ...
  async loadExistingMappings() {
    try {
      ...
      if (cached.mappings) {
        ...
        console.log(
          `📋 Loaded ${Object.keys(cached.mappings).length} existing mappings`
        );
      }
    } catch (error) {
  ...
  calculatePathSimilarity(target, existing) {
    ...
    const commonNumbers = targetNumbers.filter((n) =>
  ...
  determineImageDirectory(filename, originalPath) {
    ...
    if (
      ...
      fn.includes('comparison') ||
      fn.includes('analysis')
    ) {
      return 'reviews';
    }
  ...
  async applyPathMappings(dryRun = false) {
    console.log(
      `${dryRun ? '🔍' : '🔧'} ${dryRun ? 'Analyzing' : 'Applying'} path mappings...`
    );
    ...
    console.log(
      `${dryRun ? '📊' : '✅'} ${fixes.length} path ${dryRun ? 'fixes identified' : 'mappings applied'}`
    );
    return fixes;
  }
  ...
  async applyMappingsToFile(filePath, dryRun = false) {
    try {
      ...
      for (const [originalPath, mapping] of this.pathMappings.entries()) {
        if (
          originalContent.includes(originalPath) &&
          mapping.mappedPath !== originalPath
        ) {
