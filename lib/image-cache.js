...
class ImageCacheManager {
  constructor(options = {}) {
    this.cacheDir =
      options.cacheDir || path.join(__dirname, '..', '.cache', 'images');
  ...
  async cleanupCache() {
    ...
    try {
      ...
      console.log(
        `✅ Cache cleanup complete: ${deletedCount} files deleted, ${(freedSpace / 1024 / 1024).toFixed(2)}MB freed`
      );
    } catch (error) {
  ...
  async clearCache() {
    try {
      await Promise.all([
        fs
          .rmdir(path.join(this.cacheDir, 'images'), { recursive: true })
          .catch(() => {}),
        fs
          .rmdir(path.join(this.cacheDir, 'metadata'), { recursive: true })
          .catch(() => {}),
      ]);
