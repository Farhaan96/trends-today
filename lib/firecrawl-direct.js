...
class FirecrawlDirect {
  ...
  async scrapeAppleProduct(productName) {
    ...
    try {
      ...
      if (data.success && data.data?.markdown) {
        ...
        console.log(`📄 Content length: ${data.data.markdown.length} characters`);
  ...
  extractArticles(markdown) {
    ...
    return articleHeaders.slice(0, 10).map(header => header.replace(/^#+\s+/, ''));
  }

  generateUIInsights(markdown) {
    return {
      hasHeroSection: markdown.toLowerCase().includes('hero') || markdown.includes('featured'),
      hasSidebar: markdown.toLowerCase().includes('sidebar') || markdown.includes('latest'),
      hasCategories: markdown.toLowerCase().includes('reviews') && markdown.toLowerCase().includes('news'),
