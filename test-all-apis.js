...
async function testUnsplash() {
  ...
  try {
    const response = await fetch(`https://api.unsplash.com/search/photos?query=iphone&per_page=1&client_id=${apiKey}`);
    
    if (response.status === 200) {
      const data = await response.json();
      console.log(`${colors.green}✅ Unsplash API working!${colors.reset}`);
      if (data.results && data.results[0]) {
        console.log(`   Found: ${data.results[0].description || 'iPhone image'}`);
        console.log(`   URL: ${data.results[0].urls?.regular}`);
      }
      return true;
    } else {
      console.log(`${colors.red}❌ Unsplash error: ${response.status}${colors.reset}`);
      return false;
    }
  } catch (error) {
    console.log(`${colors.red}❌ Unsplash error: ${error.message}${colors.reset}`);
    return false;
  }
}
...
async function testPexels() {
  ...
  try {
    const response = await fetch('https://api.pexels.com/v1/search?query=smartphone&per_page=1', {
    ...
    if (response.status === 200) {
      ...
      console.log(`${colors.red}❌ Pexels error: ${response.status}${colors.reset}`);
      return false;
    }
  } catch (error) {
    console.log(`${colors.red}❌ Pexels error: ${error.message}${colors.reset}`);
    return false;
  }
}
...
async function testOpenAI() {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      console.log(`   Models: ${hasGPT4 ? 'GPT-4 ✓' : 'GPT-3.5'} ${hasDalle ? '| DALL-E ✓' : ''}`);
      return true;
    } else {
      console.log(`${colors.red}❌ OpenAI error: ${response.status}${colors.reset}`);
      return false;
    }
  } catch (error) {
...
async function testGemini() {
  ...
  try {
    ...
    if (response.status === 200) {
      const data = await response.json();
      console.log(`${colors.green}✅ Google Gemini API working!${colors.reset}`);
      
      // Check for Nano Banana model
      const hasNanoBanana = data.models?.some(model => 
        model.name?.includes('gemini-2.5-flash-image') || 
        model.name?.includes('gemini-2-5-flash-image')
      );
      
      if (hasNanoBanana) {
        console.log(`   ${colors.green}✅ Nano Banana (Gemini 2.5 Flash Image) available!${colors.reset}`);
      } else {
      ...
      console.log(`${colors.red}❌ Google API error: ${response.status}${colors.reset}`);
    ...
    console.log(`${colors.red}❌ Google API error: ${error.message}${colors.reset}`);
    return false;
  }
}
...
async function testNanoBananaGeneration(apiKey) {
  ...
  try {
    ...
    if (response.status === 200) {
      console.log(`   ${colors.green}✅ Image generation request successful!${colors.reset}`);
      console.log(`   Cost: $0.039 per image`);
    } else if (response.status === 404) {
      console.log(`   ${colors.yellow}⚠️  Model not available yet or different endpoint needed${colors.reset}`);
    } else {
      console.log(`   ${colors.yellow}⚠️  Generation test returned: ${response.status}${colors.reset}`);
    }
  } catch (error) {
    console.log(`   ${colors.yellow}⚠️  Generation test error: ${error.message}${colors.reset}`);
  }
}
...
async function testPerplexity() {
  ...
  try {
    const response = await fetch('https://api.perplexity.ai/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'sonar',
        messages: [{ role: 'user', content: 'What is the latest iPhone model in 2025?' }],
        max_tokens: 50
      })
    });
    ...
    if (response.status === 200) {
      ...
      console.log(`${colors.red}❌ Perplexity error: ${response.status}${colors.reset}`);
      return false;
    }
  } catch (error) {
    console.log(`${colors.red}❌ Perplexity error: ${error.message}${colors.reset}`);
    return false;
  }
}
...
async function testFirecrawl() {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      console.log(`${colors.red}❌ Firecrawl error: ${response.status}${colors.reset}`);
      return false;
    }
  } catch (error) {
    console.log(`${colors.red}❌ Firecrawl error: ${error.message}${colors.reset}`);
    return false;
  }
}
...
async function runAllTests() {
  ...
  apis.forEach(api => {
    const status = results[api.key] ? `${colors.green}✅ Working${colors.reset}` : `${colors.red}❌ Not working${colors.reset}`;
  ...
  if (workingCount === 6) {
    ...
    console.log(`${colors.green}Your blog has full automation capabilities:${colors.reset}`);
