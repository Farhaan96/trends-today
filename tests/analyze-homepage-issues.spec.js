...
test.describe('Trends Today Homepage Issues Analysis', () => {
  test('Identify grey rev squares and image problems', async ({ page }) => {
    console.log(
      '🔍 Analyzing homepage for grey "rev" squares and image issues...'
    );
        ...
        for (let i = 0; i < elements.length; i++) {
          ...
          try {
            ...
            const articleData = await element.evaluate((el) => {
              ...
              const hasRevText =
                textContent.includes('rev') || textContent.includes('REV');

              // Check for grey elements or placeholders
              const greyElements = Array.from(el.querySelectorAll('*'))
                .filter((node) => {
                  const styles = window.getComputedStyle(node);
                  const bgColor = styles.backgroundColor;
                  const color = styles.color;
                  return (
                    bgColor.includes('128') ||
    ...
    const greyRevIssues = allArticles.filter(
      (article) => article.hasRevText || article.greyElements.length > 0
    );
      ...
      greyRevIssues.forEach((article, index) => {
        ...
        if (article.greyElements.length > 0) {
          ...
          article.greyElements.forEach((el) => {
            console.log(
              `    - ${el.tagName}.${el.className}: "${el.textContent}" (bg: ${el.backgroundColor}, color: ${el.color})`
            );
          });
        }
      });
      ...
      if (article.title) {
        ...
        if (article.images.length > 0) {
          article.images.forEach((img, imgIndex) => {
            ...
            console.log(
              `     Image ${imgIndex + 1}: ${filename} (${img.alt || 'No alt text'})`
            );
          });
        } else {
