...
class APICache {
  ...
  async loadMetadata() {
    try {
      ...
      const daysSinceReset =
        (now - metadata.apiUsage?.perplexity?.lastReset || 0) /
        (24 * 60 * 60 * 1000);
  ...
  async getCachedResult(query, type = 'research', api = 'perplexity') {
    ...
    const cacheDir =
      api === 'perplexity' ? this.perplexityDir : this.firecrawlDir;
  ...
  async cleanExpiredCache() {
    ...
    const perplexityCleaned = await cleanDirectory(
      this.perplexityDir,
      'research'
    );
    const firecrawlCleaned = await cleanDirectory(
      this.firecrawlDir,
      'firecrawl'
    );

    console.log(
      `🧹 Cleaned ${perplexityCleaned + firecrawlCleaned} expired cache entries`
    );
  ...
  async getOptimizedQuery(originalQuery, contentType = 'research') {
    ...
    return {
      ...
      remainingCalls:
        this.maxDailyCalls.perplexity - this.apiUsage.perplexity.daily,
    };
  }
  ...
  async reportUsage() {
    ...
    console.log(
      `Perplexity: ${this.apiUsage.perplexity.daily}/${this.maxDailyCalls.perplexity} calls today`
    );
    console.log(
      `Firecrawl: ${this.apiUsage.firecrawl.daily}/${this.maxDailyCalls.firecrawl} calls today`
    );
    console.log(`Cache hits: ${stats.totalCacheFiles} files cached`);
    console.log(
      `Cache efficiency: ${stats.perplexityCacheSize > 0 ? '✅ Active' : '⚠️ Empty'}`
    );
