...
class LongTailKeywordGenerator {
  ...
  generateKeywords(topic, category, options = {}) {
    ...
    if (includeQuestions) {
      this.generateQuestionKeywords(topic, category).forEach((kw) =>
        keywords.add(kw)
      );
    }

    // Generate category-specific long-tail keywords
    this.generateCategoryKeywords(topic, category, targetYear).forEach((kw) =>
      keywords.add(kw)
    );

    // Generate voice search optimized keywords
    if (includeVoiceSearch) {
      this.generateVoiceSearchKeywords(topic, category).forEach((kw) =>
        keywords.add(kw)
      );
    }
    ...
    if (includeLocal) {
      this.generateLocalKeywords(topic, category).forEach((kw) =>
        keywords.add(kw)
      );
    }

    // Generate semantic variations
    this.generateSemanticVariations(topic, Array.from(keywords)).forEach((kw) =>
      keywords.add(kw)
    );
  ...
  generateQuestionKeywords(topic, category) {
    ...
    Object.values(this.questionStarters)
      .flat()
      .forEach((starter) => {
  ...
  generateCategoryKeywords(topic, category, year) {
    ...
    patterns.templates.forEach((template) => {
      let keyword = template
        ...
        .replace(
          /{price}/g,
          ['500', '1000', '200', '100'][Math.floor(Math.random() * 4)]
        );
  ...
  generateSemanticVariations(topic, existingKeywords) {
    ...
    existingKeywords.forEach((keyword) => {
      Object.entries(this.semanticExpansions).forEach(
  ...
  analyzeKeywordDifficulty(keyword) {
    ...
    const hasQuestion =
      /^(what|how|why|when|where|who|is|can|does|should)/.test(keyword);
    ...
    return {
      ...
      opportunity:
        difficulty < 40 ? 'high' : difficulty < 60 ? 'medium' : 'low',
      wordCount,
      hasQuestion,
      hasYear,
    };
  ...
  determineContentType(keyword) {
    ...
    if (keyword.includes('vs') || keyword.includes('versus'))
      return 'comparison';
    if (keyword.startsWith('what') || keyword.startsWith('why'))
      return 'explainer';
    return 'article';
  }
...
if (require.main === module) {
  ...
  categories.forEach((category) => {
    ...
    keywords.forEach((kw) => {
      ...
      console.log(
        `  - "${kw}" (Difficulty: ${analysis.difficulty}, Opportunity: ${analysis.opportunity})`
      );
    });
  });
}
