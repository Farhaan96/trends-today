...
class CDPClient {
  ...
  async evaluate(jsCode, timeoutMs = this.defaultTimeoutMs) {
    ...
    try {
      ...
      if (result.exceptionDetails) {
        throw new Error(`JavaScript error: ${result.exceptionDetails.exception.description}`);
      }
  ...
  async domQuerySelectorAll(selector, contextNodeId = null) {
    ...
    try {
      ...
      throw new Error(`Failed to query all selectors "${selector}": ${error.message}`);
    }
  }
  ...
  async domGetOuterHTML(nodeId) {
    ...
    try {
      ...
      throw new Error(`Failed to get outer HTML for node ${nodeId}: ${error.message}`);
    }
  }
  ...
  async domGetAttributes(nodeId) {
    ...
    try {
      ...
      throw new Error(`Failed to get attributes for node ${nodeId}: ${error.message}`);
    }
  }
  ...
  async domGetTextContent(nodeId) {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Failed to get text content for node ${nodeId}: ${error.message}`);
      }
    }
  }
  ...
  async extractElements(selectors, options = {}) {
    const { includeAttributes = true, includeText = true, maxElements = 50 } = options;
