...
async function testGemini() {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      console.log(
        '   You can use Nano Banana (gemini-2.5-flash-image-preview) for image generation'
      );
      return true;
    } else {
...
async function testAllAPIs() {
  ...
  console.log(
    `   Unsplash: ${results.unsplash ? '✅ Working' : '❌ Not working'}`
  );
  console.log(`   Pexels: ${results.pexels ? '✅ Working' : '❌ Not working'}`);
  console.log(
    `   Google Gemini: ${results.gemini ? '✅ Working' : 'ℹ️  Not configured'}`
  );
  ...
  if (workingAPIs > 0) {
    ...
    console.log(
      '   Run: npm run agents:batch to generate content with proper images'
    );
  }
