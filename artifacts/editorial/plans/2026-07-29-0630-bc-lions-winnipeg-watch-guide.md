...
3. If the plan passes, draft one 300-to-600-word sports bulletin at `artifacts/editorial/release-candidates/sports/bc-lions-winnipeg-july-30-watch-guide.mdx`. Match the frontmatter contract of the existing sports release candidate, including `status: release-candidate`, `brandSafety: standard`, `sponsorshipStatus: editorial`, `commercialIntent: none` with a non-empty `commercialFitReason`, and a non-empty `reportingMethod`.
