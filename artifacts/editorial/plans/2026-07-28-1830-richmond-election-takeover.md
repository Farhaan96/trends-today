...
6. Copy the accepted uncommitted GPT artifact to a recoverable absolute path outside every worktree. Add a new detached review worktree at `EVIDENCE_SHA` and keep it byte-clean. Run `C:\Users\farha\.codex\scripts\invoke-claude-review.ps1` there with `-ExpectedSha EVIDENCE_SHA -PrimaryModel claude-opus-5 -DisableFallback`; pass the GPT artifact's absolute external path and its full JSON body inline in the Opus prompt. The prompt must require independent candidate/source/image review, exact candidate hash, exact repository SHA, and explicit reconciliation with GPT. No GPT file, Opus file, build output, or other mutation may land in the detached worktree before or during the runner.
