...
- The saved Opus prompt contains no double-quote character, is shorter than 30,000 characters, and does not end in a backslash. The two-hop PowerShell-to-native argv probe exits 0 with `status: success`; its native dump contains thirteen arguments and proves the prompt arrives byte-identically as one `-p` value by matching UTF-8 SHA-256. The accepted Opus review text literally attests the independently recomputed repaired candidate SHA-256, the external GPT artifact SHA-256 as a transport receipt, and `gpt-5.6-sol`.
