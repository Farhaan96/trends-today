...
async function generateDalleImages() {
  ...
  try {
    ...
    if (!isHealthy) {
      console.log(
        '❌ DALL-E health check failed. Please check your API key and credits.'
      );
      return;
    }
    ...
    for (const category of categories) {
      ...
      try {
        ...
        console.log(
          `📁 Processing ${category} category (${mdxFiles.length} articles)...`
        );
    ...
    console.log(
      `   Success rate: ${totalArticles > 0 ? Math.round((generatedImages / totalArticles) * 100) : 0}%`
    );
  } catch (error) {
    ...
    console.log(
      '4. Ensure you have write permissions to the content directory'
    );
  }
}
