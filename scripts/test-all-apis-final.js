...
async function runTests() {
  ...
  if (successRate >= 70) {
    console.log(
      '✨ Your API setup is healthy! Core functionality will work well.'
    );
  } else if (successRate >= 50) {
    console.log('⚠️  Some APIs need attention, but core features should work.');
  } else {
    console.log(
      '🔧 Several APIs need configuration. Check the failed APIs above.'
    );
  }
  ...
  const workingCore = results.working.filter((api) =>
    coreAPIs.includes(api.name)
  ).length;
  console.log(
  ...
  if (workingCore === coreAPIs.length) {
    console.log(
      '   ✅ All essential APIs for content generation are operational!'
    );
  }
}
