...
async function runTests() {
  ...
  await testAPI('DALL-E 3 (OpenAI)', async () => {
    ...
    if (response.status === 400 && response.data.error?.code === 'billing_hard_limit_reached') {
  ...
  const envVars = {
    'PERPLEXITY_API_KEY': process.env.PERPLEXITY_API_KEY ? `✅ Set (${process.env.PERPLEXITY_API_KEY.substring(0, 10)}...)` : '❌ Not set',
    'OPENAI_API_KEY': process.env.OPENAI_API_KEY ? `✅ Set (${process.env.OPENAI_API_KEY.substring(0, 10)}...)` : '❌ Not set',
    'UNSPLASH_ACCESS_KEY': process.env.UNSPLASH_ACCESS_KEY ? `✅ Set (${process.env.UNSPLASH_ACCESS_KEY.substring(0, 10)}...)` : '❌ Not set',
    'PEXELS_API_KEY': process.env.PEXELS_API_KEY ? `✅ Set (${process.env.PEXELS_API_KEY.substring(0, 10)}...)` : '❌ Not set',
    'GOOGLE_AI_API_KEY': process.env.GOOGLE_AI_API_KEY ? `✅ Set (${process.env.GOOGLE_AI_API_KEY.substring(0, 10)}...)` : '❌ Not set',
    'FIRECRAWL_API_KEY': process.env.FIRECRAWL_API_KEY ? `✅ Set (${process.env.FIRECRAWL_API_KEY.substring(0, 10)}...)` : '❌ Not set',
  ...
  if (results.failed.length > 0) {
    ...
    results.failed.forEach(api => {
      if (api.error.includes('401') || api.error.includes('Unauthorized')) {
        ...
        console.log(`   • ${api.name}: Check billing/usage limits on your account`);
