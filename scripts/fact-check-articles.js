    ...
    const response = await axios.post(
      'https://api.perplexity.ai/chat/completions',
      {
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a fact-checker. Verify the following claim and provide sources. Be concise and accurate.'
          },
          {
            role: 'user',
...
async function extractKeyFacts(content) {
  ...
  sentences.forEach(sentence => {
    if (keywordPatterns.some(pattern => pattern.test(sentence)) && sentence.length > 20) {
...
async function processArticle(filePath) {
  try {
    ...
    for (let i = 0; i < facts.length; i++) {
      const fact = facts[i];
      console.log(`   🔍 Checking fact ${i + 1}/${facts.length}: "${fact.substring(0, 50)}..."`);
      ...
      if (result) {
        ...
        if (result.toLowerCase().includes('false') || result.toLowerCase().includes('incorrect')) {
          needsUpdate = true;
    ...
    if (citations.size > 0 && !body.includes('## Sources') && !body.includes('## References')) {
      console.log(`   📚 Adding ${citations.size} citations to article`);
      
      const citationSection = `\n\n## Sources\n\n${Array.from(citations).map((url, i) => `${i + 1}. [${url}](${url})`).join('\n')}`;
      
      const updatedContent = matter.stringify(body + citationSection, frontmatter);
...
async function main() {
  ...
  for (const articlePath of articlePaths.slice(0, 3)) { // Process first 3 articles to save API costs
