...
const UNSPLASH_ACCESS_KEY =
  ...
  'K64i6SmHPXzEcZBqjlCsj7T6YKItzLEflhgCVOXOBLo';
const PEXELS_API_KEY =
  process.env.PEXELS_API_KEY ||
  'DCebKJ0JgqDBIoHEP4HQAP43c9Y0xKQ1ej8uhcaYxCMDQJVRvz5wQsZe';
...
const articleImageQueries = {
  ...
  'ai-settlement-anthropic': [
    'legal document signing',
    'copyright law books',
    'AI ethics meeting',
  ],
  'google-ai-mode-languages': [
  ...
  'samsung-galaxy-s25': [
    ...
    'android smartphone premium',
    'mobile AI features',
  ],

  // Reviews
  'apple-vision-pro': [
    'VR headset futuristic',
    'spatial computing',
  ...
  // Other categories
  'scientists-discover-planet': [
    'exoplanet discovery',
    'space telescope',
    'alien world',
  ],
  'psychology-procrastinate': [
    'time management',
...
async function getPexelsImage(query) {
  try {
    ...
    if (response.data.photos && response.data.photos.length > 0) {
      ...
      const randomIndex = Math.floor(
        Math.random() * Math.min(10, response.data.photos.length)
      );
...
async function generateUniqueImage(article, category) {
  ...
  const placeholderQuery = encodeURIComponent(
    `${article.title} ${category} technology`
  );
  return {
