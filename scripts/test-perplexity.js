...
console.log('API Key prefix:', process.env.PERPLEXITY_API_KEY ? process.env.PERPLEXITY_API_KEY.substring(0, 15) + '...' : 'NOT FOUND');
...
async function testModel(model) {
  try {
    ...
    const error = e.response?.data?.error?.message || e.response?.data?.message || e.response?.statusText || e.message;
    console.log(`❌ Failed with ${model}:`);
    console.log(`   Error: ${error}`);
    
    // If it's a 401, the key is invalid
    if (e.response?.status === 401) {
      console.log('   🔑 This appears to be an authentication issue. Your API key may be invalid.');
    }
    return false;
  }
}
...
async function runTests() {
  ...
  if (!found) {
    ...
    console.log('\n📝 Next steps:');
    console.log('1. Verify your API key at: https://www.perplexity.ai/settings/api');
    ...
    if (process.env.PERPLEXITY_API_KEY) {
      ...
      console.log('Make sure this matches what you see in the Perplexity dashboard.');
    }
  }
}
