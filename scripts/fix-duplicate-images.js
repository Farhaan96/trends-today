...
const imageMapping = {
  ...
  'iphone-16-pro-max-review-surprising': 'https://picsum.photos/id/406/1200/800', // iPhone 16 surprising
  'iphone-16-pro-max-review-the-ai': 'https://picsum.photos/id/490/1200/800', // iPhone 16 AI
  'iphone-16-pro-max-review-the-review': 'https://picsum.photos/id/535/1200/800', // iPhone 16 echo
...
async function processArticle(filepath) {
  try {
    ...
    if (frontmatter.image !== imageUrl) {
      ...
      modified = true;
      console.log(`  ✅ Set unique image: ${imageUrl.split('/id/')[1]?.split('/')[0] || 'unique'}`);
    }
    ...
    if (frontmatter.images.hero !== imageUrl || frontmatter.images.featured !== imageUrl) {
  ...
  function checkFile(filepath) {
    ...
    if (imageUrl) {
      ...
      if (imageUrls.has(baseUrl)) {
        console.log(`  ⚠️ Duplicate found: ${path.basename(filepath)} shares image with ${imageUrls.get(baseUrl)}`);
        return false;
      }
