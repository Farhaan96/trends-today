...
export async function getHomepageContent(): Promise<HomepageContent> {
  ...
  const psychologyArticles = allArticles.filter(
  ...
  return {
    ...
    latestReviews: techArticles
      .filter((a) => a.frontmatter.title?.toLowerCase().includes('review'))
      .slice(0, 4),
    latestComparisons: techArticles
      .filter((a) => a.frontmatter.title?.toLowerCase().includes('vs'))
      .slice(0, 2),
    bestGuides: [
      ...scienceArticles,
      ...cultureArticles,
...
async function getArticlesFromCategoryDir(
  contentDir: string,
  category: string
): Promise<Article[]> {
  try {
