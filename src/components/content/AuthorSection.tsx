...
export async function getAuthorArticles(authorName: string, exclude?: string) {
  ...
  return posts
    .filter(post => {
      const postAuthor = typeof post.frontmatter.author === 'string' 
                ...
                {moreArticles.map((article, index) => (
                  <li key={index}>
                    ...
                    <span className="text-xs text-gray-500 ml-2">
                      {new Date(article.frontmatter.publishedAt || article.frontmatter.datePublished || '').toLocaleDateString('en-US', { 
                        month: 'short', 
                        day: 'numeric' 
                      })}
                    </span>
