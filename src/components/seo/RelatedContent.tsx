      ...
      case 'guide':
        return (
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M3 5a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-.293.707L12 12.414V15a1 1 0 01-.293.707l-2 2A1 1 0 018 17v-4.586L3.293 7.707A1 1 0 013 7V5z"
              clipRule="evenodd"
            />
          </svg>
        );
    ...
    <Link href={link.url} className="group block">
      <div className="bg-white border border-gray-200 rounded-lg p-4 h-full hover:shadow-md transition-shadow duration-200 hover:border-blue-300">
        <div className="flex items-center mb-2">
          <span
            className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getTypeColor(link.type)}`}
          >
...
export function TopicClusterNav({
  ...
  const clusterPages = clusterLinks.filter(
    (link) => link.relevanceScore !== 100
  );
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {clusterPages.map((link, index) => (
              ...
              >
                <span
                  className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium mr-2 ${getTypeColor(link.type)}`}
                >
                  {getTypeIcon(link.type)}
                </span>
      ...
      <ol className="flex items-center space-x-2 text-sm text-gray-800">
        {items.map((item, index) => (
          <li key={index} className="flex items-center">
            {index > 0 && (
              <svg
                className="w-4 h-4 mx-2 text-gray-900"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
