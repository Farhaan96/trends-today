...
export async function POST(request: NextRequest) {
  try {
    ...
    const combinedResearch = combineResearchData(
      perplexityData,
      firecrawlData,
      productName
    );
...
function getManufacturerUrl(productName: string): string | null {
  ...
  if (
    ...
    product.includes('ipad') ||
    product.includes('apple')
  ) {
    if (product.includes('iphone 16'))
      return 'https://www.apple.com/iphone-16/';
    if (product.includes('iphone 15'))
      return 'https://www.apple.com/iphone-15/';
  ...
  if (product.includes('galaxy') || product.includes('samsung')) {
    if (product.includes('s24') || product.includes('s25'))
      return 'https://www.samsung.com/us/smartphones/galaxy-s/';
  ...
  if (perplexityData?.content) {
    ...
    competitors.forEach((competitor) => {
      if (
        ...
        !productName.toLowerCase().includes(competitor)
      ) {
...
function extractSpecifications(content: string) {
  ...
  const patterns = [
    {
      key: 'display',
      regex: /(\d+\.?\d*[-"]?\s?inch.*?(?:display|screen|oled|lcd))/i,
    },
    {
      key: 'processor',
      regex: /(a\d+\s+(?:pro\s+)?chip|snapdragon\s+\d+|tensor\s+g\d+)/i,
    },
