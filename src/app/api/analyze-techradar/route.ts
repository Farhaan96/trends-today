...
function analyzeContent(markdown: string) {
  ...
  return {
    ...
    hasHeroSection:
      markdown.toLowerCase().includes('hero') || markdown.includes('featured'),
    hasSidebar:
      markdown.toLowerCase().includes('sidebar') ||
      markdown.toLowerCase().includes('latest'),
    contentDensity:
...
function extractArticles(markdown: string) {
  ...
  while ((match = headingRegex.exec(markdown)) !== null) {
    ...
    if (
      title.length > 10 &&
      !title.match(/^(Home|News|Reviews|About|Contact|Menu|Search)$/i)
    ) {
...
function extractNavigation(markdown: string) {
  ...
  while ((match = navRegex.exec(markdown)) !== null) {
    ...
    if (
      linkText.match(
        /^(Reviews|News|Buying|Best|Compare|How to|Guide|Analysis|Opinion)/i
      )
    ) {
...
function determineArticleType(title: string): string {
  ...
  if (titleLower.includes('review') || titleLower.includes('tested'))
    return 'review';
  if (titleLower.includes('vs') || titleLower.includes('compare'))
    return 'comparison';
  if (titleLower.includes('best') || titleLower.includes('guide'))
    return 'buying-guide';
  if (
    titleLower.includes('leak') ||
    titleLower.includes('rumor') ||
...
function generateUIInsights(markdown: string, _metadata: unknown) {
  ...
  if (markdown.toLowerCase().includes('compare'))
    contentTypes.add('Comparisons');
...
function generateRecommendations(analysis: any, uiInsights: any) {
  ...
  if (analysis.hasHeroSection) {
    recommendations.push(
      '✅ Keep hero section - effective for featuring main content'
    );
  } else {
    recommendations.push(
      '💡 Consider adding a hero section for featured articles'
    );
  }

  if (analysis.hasSidebar) {
    recommendations.push(
      '✅ Sidebar approach works well - keep latest news/popular content'
    );
  }

  if (analysis.contentDensity === 'high') {
    recommendations.push(
      '⚖️ High content density - consider better visual hierarchy'
    );
  }

  if (
    uiInsights.contentTypes.includes('Reviews') &&
    uiInsights.contentTypes.includes('News')
  ) {
    recommendations.push(
      '✅ Reviews + News mix is effective - maintain balance'
    );
  }
