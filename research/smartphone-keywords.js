...
const keywords = [
  ...
  {
    keyword: 'smartphone buying guide',
    volume: 3600,
    kd: 62,
  ...
  {
    keyword: 'best camera phone 2025',
    volume: 7200,
    kd: 71,
    intent: 'commercial',
    cpc: 2.89,
...
keywords.forEach((kw, i) => {
  ...
  console.log(
    `   Volume: ${kw.volume.toLocaleString()}/mo | Difficulty: ${kw.kd}/100 | CPC: $${kw.cpc} | Intent: ${kw.intent}`
  );
  console.log('');
});
...
console.log(
  '- High-volume opportunities: "best smartphones 2025", "iPhone vs Samsung"'
);
...
const csv = [
  ...
  ...keywords.map(
    (kw) => `"${kw.keyword}",${kw.volume},${kw.kd},${kw.cpc},${kw.intent}`
  ),
].join('\n');
